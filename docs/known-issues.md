...
- **ACM 串口日志为空（2026-08-30，已定位并规避，根治需内核重建）**：`cat /dev/ttyACM0` 抓不到任何内核输出。三个叠加原因：
  ...
  - 当前规避（已固化进 EGL 启动器 `boot/egl-test/sbin/run_recovery.sh` 的 `start_acm_console`）：重建字符设备节点 + `cat /dev/kmsg > /dev/ttyGS0` 阻塞转发内核日志 + 在 ttyGS0 上起一个调试 shell。新镜像 `boot-oneplus3-pmos612-v74dtb-egl-acm.img`（`581a94b2…`）开机即自动生效，主机端验证过 72 KB 内核日志实时到达。
