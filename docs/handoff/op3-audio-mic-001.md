...
Recommended next experiment: Transient-boot the corrected image `boot-oneplus3-pmos612-own-dtb-audio-diagnostic.img` (SHA256 `2625ed52fd90ac5e7162155b13e2040833516795aa568092c4464bb5e0d62a9f`). The launcher automatically runs `route.sh diagnose` and persists the result at `/newroot/var/log/op3-audio-initramfs.log`; no sda15/rootfs change is made. Only if the card and MultiMedia3 PCMs exist, use owner-directed `route.sh speaker` plus 30-second tinyplay, then `route.sh mic` and 48 kHz/16-bit/mono tinycap.
```
