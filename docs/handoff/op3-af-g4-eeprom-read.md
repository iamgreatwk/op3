...
`0x50`，默认读取 `0x0000..0x002f`。它通过 `I2C_RDWR` 发送两字节 EEPROM 地址指针
后立即读取数据；不发送 EEPROM 数据写入命令，不扫描其它地址。输出原始十六进制，
...
本次已完成源码静态审计和工具构建准备；由于设备 SSH 在 banner 阶段超时，尚未
执行手机端读操作。没有刷机、没有改 DTS、没有写 EEPROM，也没有重新编译内核或
Buildroot。下一 PASS 条件是：在 CCI0/地址 `0x50` 上完成一次成功的只读事务，
保存完整原始字节和 dmesg，并确认没有 CCI 错误、相机重启或异常温升。
