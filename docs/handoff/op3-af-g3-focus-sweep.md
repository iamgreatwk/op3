...
## 显示器高对比目标复测：仍未发现光学焦点峰值（2026-09-13）

在保持同一硬件和命令不变的情况下，将手机稳定对准显示器上的文字，重新进行一次
干净启动的 G3。结果仍为 `helper-rc=0`、`frames_good=128`、`frames_error=0`，
五档 VCM 命令和三帧采样全部完成；本次没有 `VFE overflow`、SMMU fault、CCI
错误或重启。

显示器目标的同一中央 ROI Tenengrad 三帧均值为：

```text
position  0          256        512        768        1023
score     276.411    276.623    276.980    276.144    276.346
```

最大位置差异约 `0.30%`，低于同一位置三帧的自然波动；五档对比图已保存为
`artifacts/op3-af-evidence/device-g3-display-20260913/focus-montage.png`，SHA256
为 `ec6428e4e137642043e8601301373959af35e19b8fd4810773c0f207a090172a`。图中五个
位置的文字轮廓基本相同且都偏模糊，说明当前 VCM 写事务虽然完成，但尚未证明
镜头产生了可检测的光学位移。

这比低纹理初筛更有区分力，但仍不能单凭图像排除目标距离、镜头默认位置或缺失
actuator servo/init 序列的影响；因此 `LENS-MOTION` 继续保持 `INCONCLUSIVE`，
不能进入闭环 AF 或修改 VCM 寄存器。
