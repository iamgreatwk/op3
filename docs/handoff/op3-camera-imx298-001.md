...
through the IMX298 -> CAMSS -> VFE RDI path. The test used the existing
module-only gain build and did not change the kernel image, DTB, Buildroot,
or source tree:

- mode: `1476x834`, `MEDIA_BUS_FMT_SRGGB10_1X10`, four CSI-2 lanes;
- controls: exposure `893`, analogue gain `240`;
- frame size: `1541232` bytes (`bytesperline=1848`);
- RAW SHA256: `5509aa0d3f8a6f6676c6f57ce8481ff2a0206c3bc73aca6e390e625a2fa4ec51`.

The host-side preview `/tmp/op3-color-wb.png` was generated by unpacking
RAW10, RGGB bilinear demosaicing, subtracting a provisional black level of
`64`, and applying gray-world white balance. It is a colour-data proof, not
an ISP/JPEG result: the frame is underexposed (`raw 62..89`) and visibly
noisy, so the white-balance coefficients are not calibration data.

A separate `gain=960` attempt was not accepted. It produced repeated
`VFE0 rdi0 overflow` and ARM SMMU translation faults, and a subsequent frame
wait timed out. The stable colour result is therefore the `gain=240` frame;
future gain work must first isolate the CAMSS/VFE buffer fault.
