...
known-good v100 image. That image is provenance only; current rebuilds use a
Buildroot-generated initramfs and separately built kernel/DTB.
...
| Ramdisk | Buildroot-generated gzip-compressed initramfs |
...
The old kernel payload, appended DTB, boot header, load addresses, cmdline,
and ramdisk are not consumed by the current recovery build. The historical
standalone ramdisk remains outside GitHub only as provenance. The current
Buildroot target receives selected Qualcomm, A530, and ath10k files from
separately pinned external inputs.

## Safe reuse rules

- Use `scripts/pack-boot.sh` with a built `Image.gz`,
  `msm8996-oneplus3.dtb`, and
