...
| Next action | OP3-INITRD-SPLIT-001: keep early ADSP/SLPI and small GPU firmware in initramfs; move deferred modem/MBA, Venus and ath10k firmware to rootfs and prove loading only after `/newroot` is available. Separate open lines: GPU regulator nodes in DTB (hard-reset root cause) and pmOS 6.12 kernel rebuild with `CONFIG_U_SERIAL_CONSOLE=y`; 7.x resume requires physical MSM8996 UART. |
