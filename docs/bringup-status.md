...
| Next action | OP3-INITRD-FW-001: migrate only MSM8996 initramfs firmware to explicit sources/SHA256 while all OP3-BOOT-043 controls stay fixed. Separate open lines: GPU regulator nodes in DTB (hard-reset root cause) and pmOS 6.12 kernel rebuild with `CONFIG_U_SERIAL_CONSOLE=y`; 7.x resume requires physical MSM8996 UART. |
