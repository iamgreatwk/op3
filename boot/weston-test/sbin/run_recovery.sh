#!/bin/sh
# ============================================================
# OnePlus 3 Weston gate launcher (initramfs side).
#
# Deliberately tiny. It waits for the persistent sda15 root filesystem and then
# hands control to /newroot/opt/op3-weston/run.sh, which lives on sda15. The Mesa
# stack, the Weston test program, the test logic and the logs therefore live on
...
LOG=/var/log/op3-weston.log
PERSIST_DIR=/newroot/var/log
PERSIST=$PERSIST_DIR/op3-weston.log
BUNDLE_DIR=/newroot/opt/op3-weston
...
log() {
	line="$(date '+%H:%M:%S') $*"
	printf '%s\n' "$line" >> "$LOG"
	{ printf '%s\n' "op3-weston: $line" > /dev/kmsg; } 2>/dev/null
}
...
if [ ! -f "$BUNDLE_RUN" ]; then
	...
	snapshot "before Weston test"
	"$BUNDLE_RUN" >> "$LOG" 2>&1
	log "bundle exit=$?"
	snapshot "after Weston test"
fi
